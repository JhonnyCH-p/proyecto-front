  ...
  return (
    <div className="group bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition-shadow duration-300 flex flex-col">
      ...
      <div className="p-4 flex flex-col flex-grow">
        ...
        <h3 className="text-lg font-bold text-gray-800"> {asesor.nombre} {asesor.apellido}  </h3>

        <p className="text-sm text-gray-500 mt-0.5">  Agente · {asesor.especialidad || 'Alpha Inmobiliaria'}  </p>
        ...
        <div className="mt-4">
          <Link to={`/asesores/${asesor.id}`}>
            <button className="w-full py-2.5 px-4 bg-primary text-white font-medium rounded-lg hover:bg-primary-dark transition-colors duration-300 text-sm">Ver Perfil</button>
          </Link>
        </div>
      </div>
    </div>
